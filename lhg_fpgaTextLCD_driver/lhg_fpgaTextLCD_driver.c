...
#define DEVICE_NAME			"lhg_fpgaTextLCD_driver"
...
static int lhg_fpgaTextLCD_driver_open(struct inode * inode, struct file * file)
{
	if (textlcd_usage == -1)
		return -EBUSY;
...
static int lhg_fpgaTextLCD_driver_release(struct inode * inode, struct file * file)
{
...
/* not use
static ssize_t lhg_fpgaTextLCD_driver_read(struct file * file, char * buf, size_t length, loff_t * ofs)
{
	printk("lhg_fpgaTextLCD_driver_read, \n");
	
	return 0;
}
*/

static ssize_t lhg_fpgaTextLCD_driver_write(struct file * file, const char * buf, size_t length, loff_t * ofs)
{
	int i;
	int size;
...
static DEFINE_MUTEX(lhg_fpgaTextLCD_driver_mutex);
static long lhg_fpgaTextLCD_driver_ioctl(struct file * file, unsigned int cmd, unsigned long arg)
{
	...
	default:
		mutex_unlock(&lhg_fpgaTextLCD_driver_mutex);
		return ENOTTY;
	}
...
static struct file_operations lhg_fpgaTextLCD_driver_fops = {
	.owner = THIS_MODULE,
	.open = lhg_fpgaTextLCD_driver_open,
	.release = lhg_fpgaTextLCD_driver_release,
//	.read = lhg_fpgaTextLCD_driver_read,
	.write = lhg_fpgaTextLCD_driver_write,
	.unlocked_ioctl = lhg_fpgaTextLCD_driver_ioctl,
};

static struct miscdevice lhg_fpgaTextLCD_driver = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = "lhg_fpgaTextLCD_driver",
	.fops = &lhg_fpgaTextLCD_driver_fops,
};

static int lhg_fpgaTextLCD_driver_init(void){
	printk("lhg_fpgaTextLCD_driver_init, \n");
	
	return misc_register(&lhg_fpgaTextLCD_driver);
}

static void lhg_fpgaTextLCD_driver_exit(void){
	printk("lhg_fpgaTextLCD_driver_exit, \n");

	misc_deregister(&lhg_fpgaTextLCD_driver);
	
}

module_init(lhg_fpgaTextLCD_driver_init);
module_exit(lhg_fpgaTextLCD_driver_exit);

MODULE_AUTHOR("SKKU_LHG");
MODULE_DESCRIPTION("lhg_fpgaTextLCD_driver");
